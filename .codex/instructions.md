# Codex Instructions for SEMAS

> This file is read by GitHub Copilot / Codex. For a tool-agnostic version, see
> `AGENTS.md`.
...
- Read `AGENTS.md`, `README.md`, and relevant `wiki/` notes before starting.
- For architectural or multi-file changes, present a plan to the user first.
- Record absorbed papers/repos in `wiki/` with citations.
- Record operational steps in `OPERATION_LOG.md`.
- Run tests before finalizing:
  `pytest tests/ semas/plugins/function_evolve/tests/`.
...
- Preserve existing tests unless the user explicitly asks otherwise.
